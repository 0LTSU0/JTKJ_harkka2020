...
double opt3001_get_data(I2C_Handle *i2c) {
	...
	if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cTransaction)) {
			...
			uint16_t arvo = rxBuffer[0] << 8;
			arvo = arvo + rxBuffer[1];
			...
			uint16_t valitulos_e = arvo & maski_e;
			uint16_t rekisteri_e = valitulos_e >> 12;

			uint16_t maski_r = 0b0000111111111111;
			uint16_t rekisteri_r = arvo & maski_r;

			lux = 0.01 * pow(2, rekisteri_e) * rekisteri_r;
